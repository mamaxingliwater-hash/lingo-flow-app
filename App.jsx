...
import { 
  ...
  GoogleAuthProvider, // 引入 Google 登录提供者
  signInWithPopup,    // 引入 Google 登录方法
  signOut             // 引入退出登录方法
} from 'firebase/auth';
import { 
  getFirestore, 
...
const callAI = async (prompt, apiKey, modelType = 'text') => {
  if (!apiKey) {
    ...
    return JSON.stringify({
      definition: "这是一个模拟的中文释义 (Simulated Definition). Please add an API Key in settings for real AI intelligence.",
  ...
  try {
    ...
    alert("AI 调用失败，请检查您的 API Key 或网络连接。"); 
    return null;
  }
};
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
        // 优先使用 Canvas 提供的 Custom Token 登录
        await signInWithCustomToken(auth, __initial_auth_token);
      } else {
        // 否则使用匿名登录，等待用户手动 Google 登录
  ...
  const handleGoogleSignIn = async () => {
    try {
        ...
        // 使用自定义模态而不是 alert
        alert("Google 登录成功！数据将在所有设备上实时同步。");
    } catch (error) {
        console.error("Google Sign-in Error:", error);
        // 使用自定义模态而不是 alert
        alert("Google 登录失败，请确保您已在 Firebase 控制台开启 Google 登录。");
    }
  };
